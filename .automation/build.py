...
UPDATE_DOC = "--doc" in sys.argv or RELEASE
...
if RELEASE:
    ...
    VERSION_V = VERSION
# latest management
VERSION_URL_SEGMENT = "latest" if IS_LATEST else VERSION
MKDOCS_URL_ROOT = ML_DOC_URL_BASE + VERSION_URL_SEGMENT

BRANCH = "main"
URL_ROOT = f"{ML_REPO_URL}/tree/{BRANCH}"
URL_RAW_ROOT = f"{ML_REPO_URL}/raw/{BRANCH}"
TEMPLATES_URL_ROOT = f"{URL_ROOT}/TEMPLATES"
DOCS_URL_ROOT = f"{URL_ROOT}/docs"
DOCS_URL_DESCRIPTORS_ROOT = f"{DOCS_URL_ROOT}/descriptors"
DOCS_URL_FLAVORS_ROOT = f"{DOCS_URL_ROOT}/flavors"
DOCS_URL_RAW_ROOT = f"{URL_RAW_ROOT}/docs"
REPO_HOME = os.path.dirname(os.path.abspath(__file__)) + os.path.sep + ".."
REPO_ICONS = f"{REPO_HOME}/docs/assets/icons"
REPO_IMAGES = f"{REPO_HOME}/docs/assets/images"

VERSIONS_FILE = f"{REPO_HOME}/.automation/generated/linter-versions.json"
LICENSES_FILE = f"{REPO_HOME}/.automation/generated/linter-licenses.json"
USERS_FILE = f"{REPO_HOME}/.automation/generated/megalinter-users.json"
HELPS_FILE = f"{REPO_HOME}/.automation/generated/linter-helps.json"
LINKS_PREVIEW_FILE = (
    f"{REPO_HOME}/.automation/generated/linter-links-previews.json"
)
DOCKER_STATS_FILE = f"{REPO_HOME}/.automation/generated/flavors-stats.json"
FLAVORS_DIR = f"{REPO_HOME}/flavors"
LINTERS_DIR = f"{REPO_HOME}/linters"
GLOBAL_FLAVORS_FILE = f"{REPO_HOME}/megalinter/descriptors/all_flavors.json"
...
def generate_all_flavors():
    ...
    if UPDATE_DOC is True:
        try:
            ...
            logging.warning(
                f"Connection error - Unable to update docker pull counters: {str(e)}"
            )
        except Exception as e:
            logging.warning(f"Unable to update docker pull counters: {str(e)}")
...
"""
        ...
        with open(dockerfile, "r+", encoding="utf-8") as f:
            ...
            f.write(f"{comment}\n{content}")
        # Generate action.yml
        image_release = RELEASE_TAG if RELEASE is True else DEFAULT_RELEASE
    ...
    if len(cargo_packages) > 0:
        ...
        cargo_install_command = (
            (
                ...
                + "/root/.cache/sccache"
                + (" /root/.rustup" if not keep_rustup else "")
            )
            + "\n"
...
def generate_linter_dockerfiles():
    ...
    with open(f"{REPO_HOME}/docs/standalone-linters.md", "w", encoding="utf-8") as file:
