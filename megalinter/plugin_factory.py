...
def list_plugins(request_id):
    return config.get_list(request_id, "PLUGINS", [])
...
def load_plugin(plugin):
    ...
    if not plugin.startswith("https://") and not plugin.startswith("file://"):
        raise Exception(
    ...
    try:
        if plugin.startswith("https://"):
            ...
            if not os.access(plugin_path, os.R_OK):
                plugin_path = f"/tmp/lint/{plugin_path}"
                if not os.access(plugin_path, os.R_OK):
                    raise Exception(
...
def process_install(install, request_id):
    ...
    for command in commands:
        logging.debug(f"[Plugins] Install command: {str(command)}")
